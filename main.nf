...
workflow {
    // The historical source-data stage downloads and chunks 1000 Genomes data.
    // Its published results are already committed and need not be rebuilt when
    // regenerating the self-contained GWAS fixtures.
    if (!params.skip_source_generation) {
        ...
        def vcfs_with_chr = GENERATE_EXAMPLE_GENOTYPES_VCFS.out.vcfs
            .flatten()
            .map { file ->
                def chr = file.name.toString().split("\\.")[1]
                tuple(chr, file)
            }
    ...
    // This stage is network-free and publishes only the canonical VCF and three
    // phenotype/covariate sidecars under results/fixtures.
