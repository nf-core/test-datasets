...
### `resources_remapped/cadd/`

Trimmed CADD v1.7 GRCh38 resources for the `annotate_cadd` subworkflow. `annotate_cadd` renames the reference contigs to CADD's no-`chr` convention before scoring and back afterwards, so the prescored files use the sliced local coordinates without the `chr` prefix. Only the all-prescored path is exercised: every test variant is present in the prescored files, so `CADD.sh` never touches the annotation bundle or a reference genome.

`resources_remapped/cadd/cadd_prescored.tar.gz`: `prescored/GRCh38_v1.7/no_anno/` folder holding `grch38_cadd_snvs.tsv.gz` and `grch38_cadd_indels.tsv.gz` (+ tabix indexes) -- 21 SNV and 21 indel rows carrying their real CADD v1.7 RawScore/PHRED values, extracted from the public whole-genome SNV and gnomAD indel prescored sets and remapped to the sliced local coordinates. Bind-mounted at `/cadd-scripts/data/prescored`; untar before use.

`resources_remapped/cadd/cadd_annotations.tar.gz`: empty placeholder directory bind-mounted at `/cadd-scripts/data/annotations`. The CADD module always mounts an annotation directory, but the no-anno config never reads it. Untar before use.

All three files above plus `subworkflow_fixtures/cadd_test_variants.vcf.gz` are regenerated by `scripts/build_cadd_prescored_fixture.py` (see the `scripts/` section below).
...
`subworkflow_fixtures/cadd_test_variants.vcf.gz` (+ `.tbi`): 42 variants (21 SNVs + 21 indels, 3 of each per nuclear sliced region) whose reference alleles match `reference_sliced/minimal_reference.fasta` and which are all present in `resources_remapped/cadd/cadd_prescored.tar.gz`, input to `annotate_cadd`

### `scripts/`

Helper scripts for (re)generating fixtures in this branch. Not consumed by any test.

`scripts/build_cadd_prescored_fixture.py`: regenerates the four CADD fixtures -- `resources_remapped/cadd/cadd_prescored.tar.gz`, `resources_remapped/cadd/cadd_annotations.tar.gz` and `subworkflow_fixtures/cadd_test_variants.vcf.gz` (+ `.tbi`). Runs from any working directory (repo inputs are located relative to the script), needs `tabix`/`bgzip` on PATH and Python >= 3.8 (stdlib only), takes a few seconds, and writes straight into the working tree for review. It reads `manifests/coordinate_provenance.tsv` and `reference_sliced/minimal_reference.fasta` from this repo, plus the two full CADD prescored sets passed on the command line (`--snv-prescored` whole-genome SNVs, `--indel-prescored` gnomAD indels -- ~80 GB + ~1 GB, downloaded separately from <https://cadd.gs.washington.edu/download>, each with its `.tbi`). It `tabix`-slices the sliced regions out of those sets, lifts them onto the local coordinates, picks 3 SNVs + 3 indels per nuclear contig (seed-fixed, REF-verified), and packs the result. Run it the next time CADD is updated, passing `--cadd-version`; see the script header for the exact commands and the follow-up steps in nf-core/raredisease.
