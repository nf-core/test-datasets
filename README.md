...
- homo_sapiens
  ...
  - pacbio:
    ...
    - bed:
      ...
      - genome3_fa_bogus_excluded_cnv.bed: A BED file specifying genomic regions to exclude from copy number variant (CNV) analysis when running the `hificnv` module testing on the sliced genome3.fasta reference genome.
      - genome3_fa_bogus_expected_cnv.bed: A BED file that defines the expected copy number for specific regions when testing the `hificnv` module on the sliced genome3.fasta reference.
    - fasta:
