...
- homo_sapiens
  ...
  - illumina
    ...
    - gridss:
      ...
      - gridss.properties: GRIDSS configuration properties file, suitable for testing the `gridss` modules. source: https://github.com/PapenfussLab/gridss/blob/825bc8e1bb092e9bb2ccb0c1c51dc6e7e1a922dd/src/main/resources/gridss.properties
  ...
  - vepyr:
    - cache.tar.gz: [vepyr](https://github.com/biodatageeks/vepyr) Parquet VEP cache (Ensembl release 115, GRCh38, `ensembl` cache type) trimmed to chr1:1-860893, the region of `input.vcf.gz`, and stamped with its cache identity; archived because the cache is a directory. Produced by `nf-core-module/stage-testdata.sh` in biodatageeks/vepyr.
    - input.vcf.gz: first 100 records of the GIAB HG002 GRCh38 v4.2.1 benchmark VCF (chr1), normalized with `bcftools norm -m -both`, bgzip-compressed.
    - input.vcf.gz.tbi: tabix index of `input.vcf.gz`.
    - reference.fa: GRCh38 chromosome 1, bases 1-860893, extracted from Ensembl `Homo_sapiens.GRCh38.dna.primary_assembly.fa` with `samtools faidx` (contig named `1`).
    - reference.fa.fai: `samtools faidx` index of `reference.fa`.
...
- msa_tree_pairs
  ...
  - mammals_aa_msa.fasta: protein alignment of a short protein (187 aa) in 10 mammalian species
  - mammals_codon_tree.nwk: phylogenetic tree for mammals_codon/aa_msa.fasta
  - mammals_codon_msa_small.fasta: in frame codon alignment of a very short protein (5 sequences, 10 codons)
  - mammals_aa_msa_small.fasta: protein alignment of a very short protein (5 sequences, 10 aa)
  - mammals_codon_tree_small.nwk: phylogenetic tree for mammals_codon/aa_msa_small.fasta
- newick
